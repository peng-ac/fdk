...
def strong_scability(S = 4):
	...
	for SCALE in [1, 2, 4, 8]:
		if S != SCALE:
			continue
		for ROWS in [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096]:
			#for i in range(0, 12):
			VOL_SIZE = 1024*SCALE
			...
			print "({}) : ".format(index)
			NPP = min(NP, NPP)
			#if NP == 4096: sub_job(NP, NPP, ROWS, VOL_SIZE, VOL_SIZE, VOL_SIZE, NU, NV, PROJS, SRC_DIR, DST_DIR)
	...
	while True:
		...
		for ROWS in [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096]:
			...
			if ROWS > NP:
				break
			NX=NY=NZ=VOL_SIZE
			#if NP == 4096: sub_job(NP, NPP, ROWS, NX, NY, NZ, NU, NV, PROJS, SRC_DIR, DST_DIR)
