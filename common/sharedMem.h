...
private:
	...
	bool Create() {
		bool bRtn = false;
		int64 id = -1;
		std::string name[3] = { "SERVER", "CLIENT", "" };
		if (type == SERVER) {
			...
			if(id != -1) {
				...
				{
					printf("%s:%s:close segment success, size = %f GB\n", hostname().c_str(), name[type - 1].c_str(), segment_size / 1024.0 / 1024.0 / 1024.0);
				}
			}
		}
		...
		if (id == -1) {
			segment_id = -1;
			printf("%s:%s:create segment failed, size = %f GB, errNo.=%d, err=%s\n", hostname().c_str(), name[type - 1].c_str(), segment_size / 1024.0 / 1024.0 / 1024.0, errno, strerror(errno));
		}
		else {
			printf("%s:%s:create segment success, size = %f GB\n", hostname().c_str(), name[type - 1].c_str(), segment_size / 1024.0 / 1024.0 / 1024.0);
			segment_id = id;
